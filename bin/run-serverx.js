...
if (fs.existsSync(SERVERX_DIR)) {
	let { stdout, stderr } = shell.exec(`cd ${SERVERX_DIR} && git fetch --dry-run`);
	if (!stdout && !stderr) {
...
fs.writeFileSync(`.env`, `SERVERX_API_KEY=${SERVERX_API_KEY}\nSERVERX_PROJECT_ID=${SERVERX_PROJECT_ID}`);

res = shell.exec('python3 -m venv venv && source venv/bin/activate && pip install --upgrade pip && pip install -r requirements.txt && ./serverx');
